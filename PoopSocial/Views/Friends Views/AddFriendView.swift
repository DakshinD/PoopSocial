...
struct AddFriendView: View {
    ...
    var filteredUsers: [User] {
            if userSearchText.isEmpty {
                return UserData.shared.allUsers.filter { $0.uid != FirebaseManager.shared.auth.currentUser?.uid }
            } else {
                return UserData.shared.allUsers.filter { $0.username.contains(userSearchText) && $0.uid != FirebaseManager.shared.auth.currentUser?.uid}
            }
        }
}
